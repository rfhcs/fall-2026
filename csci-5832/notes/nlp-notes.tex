...
Everything is just logs... But they are useful!
...
\[
  \frac{\partial L_{CE}(w,b)}{\partial w_j} = [\sigma(w \cdots x + b)-y]x_j
\]

Essentially what this is doing is taking the difference of the computed answer $(w \cdots x + b)$ and the right answer $y$  and multiplying it by the value of the input feature for the weight being adjusted $x_j$.
...
\subsection{Ok, now apply it to the multinomial case}
...
$e^z_c$ is the raw score of a single class $c$ which can be expanded to $z = \sigma(w_c \cdot x + b_c)$
